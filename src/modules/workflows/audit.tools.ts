...
import { IncidentStore } from '../compliance/compliance.resources.js';
...
export class AuditTools {
  ...
  @Tool({
    ...
    description:
      ...
      'language, generates spoken multilingual alerts, escalates critical hazards for human ' +
      'approval, and returns a regulation-cited report. Runs as a long-running MCP Task with ' +
      'streaming progress.',
  ...
  async runSiteSafetyAudit(
    ...
    for (let i = 0; i < input.images.length; i++) {
      ...
      // Human-in-the-loop: pause for supervisor approval on CRITICAL hazards.
      ...
      if (hasCritical && ctx.task) {
        ctx.task.requestInput(
          ...
          `Approve stop-work + voice alert? (yes/no)`,
        );
      }

      for (const v of found) {
        allFindings.push({ type: v.type, severity: v.severity, confidence: v.confidence });
        IncidentStore.add({ siteId: input.siteId, ...v, imageIndex: i, language });
    ...
    return {
      ...
      recommendation: critical > 0
        ? 'Immediate stop-work for critical hazards; multilingual voice alerts issued; supervisor notified.'
